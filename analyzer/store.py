...
class AnalyticsStore:
    ...
    def reset(self) -> None:
        ...
        Drops FTS triggers and table to avoid FTS5 overhead during the
        subsequent bulk insert.  Call ``rebuild_fts()`` after all data
        has been re-inserted to recreate triggers and rebuild the index.
        """
        with self._get_conn() as conn:
            conn.execute("BEGIN IMMEDIATE")
            # 1. Drop triggers so DELETE/INSERT don't touch FTS5 index.
            ...
            # 2. Delete data — fast, no triggers fire.
            ...
            # 3. Recreate empty FTS table (clean internal structures).
    ...
    def rebuild_fts(self) -> None:
        """Recreate FTS triggers and rebuild the full-text index.

        Must be called after ``reset()`` + bulk insert, otherwise
        incremental inserts won't update the FTS index.
        """
        with self._get_conn() as conn:
            conn.execute("BEGIN IMMEDIATE")
            conn.execute(
            ...
            # Rebuild FTS index from all existing conversations.
            conn.execute(
