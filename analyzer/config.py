...
class AnalyzerConfig:
    ...
    batch_size: int = 500
    min_batch_size: int = 500  # minimum batch size for adaptive sizing
    max_batch_size: int = 5000  # maximum batch size
    min_poll_interval: float = 0.5  # 最小轮询间隔（秒），过高延迟大，过低空转浪费
    max_poll_interval: int = 10  # 最大轮询退避间隔（秒），新数据到达时立即重置
...
def load_analyzer_config() -> AnalyzerConfig:
    return AnalyzerConfig(
        ...
        batch_size=int(os.getenv("ANALYZER_BATCH_SIZE", "500")),
        min_batch_size=int(os.getenv("ANALYZER_MIN_BATCH_SIZE", "500")),
        max_batch_size=int(os.getenv("ANALYZER_MAX_BATCH_SIZE", "5000")),
        min_poll_interval=float(os.getenv("ANALYZER_MIN_POLL_INTERVAL", "0.5")),
        max_poll_interval=int(os.getenv("ANALYZER_MAX_POLL_INTERVAL", "10")),
